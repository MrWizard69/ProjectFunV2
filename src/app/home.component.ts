...
import { Component, Input, OnInit, ElementRef, ViewChild, AfterViewInit, NgZone, ApplicationRef } from '@angular/core';
...
export class HomeComponent {
	...
	constructor(private zone: NgZone, public app: ApplicationRef){
		
		setTimeout(() => { //this is an experiment in taking ngZones out for better performance
			this.isLoaded = true;
			this.app.tick();
			this.playGame();
			this.app.tick();
		 ...
		 let canvas1 = <HTMLCanvasElement> document.getElementById("canvas"),
			...
			function resizeCanvas() {
				...
				if(shootStickTouch == true && exitReload == 0){
					...
					// ctx.fill();
					// ctx.closePath();
					let flicker = Math.floor((Math.random() * 30) + 1);
